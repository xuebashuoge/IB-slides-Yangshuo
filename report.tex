...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Information theory plays an important role in explaining the DL mechanism \note{DL is actually extracting patterns from a large scale complex data, it involves data commpression, }
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        \item Statistic Theory: data-dependent but architecture independent bounds \note{from worst case results to a typical case result}
...
\begin{itemize}
    ...
    \item \textbf{The cascade network forms a Markov chain}
    \begin{equation}
        Y\to X\to \hat{X}
    \end{equation}
...
\begin{frame}
    ...
    \begin{columns}
        \begin{column}{0.4\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=5.5cm]{DNN.jpg}
            \end{figure}
        \end{column}
